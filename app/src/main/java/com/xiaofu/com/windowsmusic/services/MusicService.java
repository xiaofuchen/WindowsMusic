package com.xiaofu.com.windowsmusic.services;
...
import com.xiaofu.com.windowsmusic.R;
import com.xiaofu.com.windowsmusic.locatMusic.Song;
import com.xiaofu.com.windowsmusic.utils.ProgressTextUtils;
import com.xiaofu.com.windowsmusic.utils.SPUtils;
import com.xiaofu.com.windowsmusic.view.FloatWindow;

import java.io.IOException;
import java.util.List;

import static com.xiaofu.com.windowsmusic.MusicApp.getGson;
import static com.xiaofu.com.windowsmusic.services.AudioState.STATE_PAUSE;
import static com.xiaofu.com.windowsmusic.services.AudioState.STATE_PLAYING;
import static com.xiaofu.com.windowsmusic.services.AudioState.STATE_PREPARE;
import static com.xiaofu.com.windowsmusic.services.AudioState.STATE_READY;
import static com.xiaofu.com.windowsmusic.services.AudioState.STATE_STOP;
import static com.xiaofu.com.windowsmusic.utils.SPUtils.MUSIC_DATA_MARK;
...
public class MusicService extends Service implements PlayerInterface, View.OnClickListener, SeekBar.OnSeekBarChangeListener, MediaPlayer.OnCompletionListener, MediaPlayer.OnPreparedListener, MediaPlayer.OnBufferingUpdateListener {
    ...
    private AudioState audioState = STATE_STOP;
...
//    private String dis_names[] = null;
    ...
    private Song markPlayingMusic;//记录当前播放的歌曲信息，用于退出时候保存数据，下次进入时候还原播放
    private boolean isFirst = true;//是否为第一次进入来播放
    ...
    public class MusicBinder extends Binder {
        ...
        public void setSongList(List<Song> list){
            MusicService.this.mSongList = list;
            Log.e(TAG,"mSongList--"+mSongList.toString());
            Log.e(TAG,"mSongList.size()--"+mSongList.size());
            audioStart();
        }
    }
    ...
    public void onCreate() {
        super.onCreate();
        initFloatWindow();
//        dis_names = this.getResources().getStringArray(dis_names);
//        res_names = this.getResources().getStringArray(res_names);
    }
    ...
    public void onClick(View view) {
        if(animatorState == AnimatorState.STATE_END) {
            if(null != floatWindow){
                floatWindow.setLastTouchTimeMillisPlayView();
            }
            switch (view.getId()) {
    ...
    private void audioPrepare() {
        try {
            ...
            try {
                if(isFirst){
                    markPlayingMusic = MusicApp.getGson().fromJson((String)SPUtils.get(MUSIC_DATA_MARK,""),Song.class);
                    if(null == markPlayingMusic){
                        markPlayingMusic = mSongList.get(position);
                    }
                }else {
                    markPlayingMusic = mSongList.get(position);
                }
                ...
                Log.e(TAG,"markPlayingMusic-"+markPlayingMusic.toString());
                mediaPlayer.setDataSource(markPlayingMusic.getFileUrl());
                textDisplay.setText(markPlayingMusic.getFileName());  //  设置显示
            }
    ...
    public void onPrepared(MediaPlayer mediaPlayer) {
        ...
        if(isFirst){
            isFirst = false;
            mediaPlayer.seekTo(markPlayingMusic.getCurrentPosition());
            onPlayButtonClick();
        }
    }
    ...
    public void onDestroy() {
        ...
        if(null != markPlayingMusic){
            if(null != mediaPlayer) {
                markPlayingMusic.setCurrentPosition(mediaPlayer.getCurrentPosition());
            }
            Log.e(TAG,"onDestroy() markPlayingMusic-"+markPlayingMusic.toString());
            SPUtils.put(MUSIC_DATA_MARK, getGson().toJson(markPlayingMusic,Song.class));
        }
        super.onDestroy();
    }
